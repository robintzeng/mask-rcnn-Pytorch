...
class IA_roi_heads(RoIHeads):
  def forward(self,
        ...
        if self.training:
            ...
            # ------------------------ batch ---------------------
            box_features_before_after = torch.cat((box_features_tmp, box_features_tmp * mask_all), dim=0)
            box_features_before_after = self.box_head(box_features_before_after)
            cls_score_before_after, _ = self.box_predictor(box_features_before_after)
            ...
            prepare_mask_fg_num = labels.nonzero().size(0)
            ...
            fg_index = torch.where(labels > 0, torch.ones(change_vector.shape).cuda(), torch.zeros(change_vector.shape).cuda())
            bg_index = 1 - fg_index
