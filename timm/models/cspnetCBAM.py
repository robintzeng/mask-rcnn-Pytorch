...
__all__ = ['CBAMCspNet']  # model_registry will add each entrypoint fn to this
...
class CBAMBottleneck(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, dilation=1, bottle_ratio=0.25, groups=1,
                 ...
                 attn_layer='cbam', aa_layer=None, drop_block=None, drop_path=None):
...
class CBAMCrossStage(nn.Module):
    """Cross Stage."""
    ...
    def __init__(self, in_chs, out_chs, stride, dilation, depth, block_ratio=1., bottle_ratio=1., exp_ratio=1.,
                 ...
                 block_fn=CBAMBottleneck, **block_kwargs):
        super(CBAMCrossStage, self).__init__()
...
class CBAMCspNet(nn.Module):
    ...
    def __init__(self, cfg, in_chans=3, num_classes=1000, output_stride=32, global_pool='avg', drop_rate=0.,
                 ...
                 zero_init_last_bn=True, stage_fn=CBAMCrossStage, block_fn=CBAMBottleneck):
        super().__init__()
...
def _create_cbamcspnet(variant, pretrained=False, **kwargs):
    cfg_variant = variant.split('_')[0]
    return build_model_with_cfg(
        CBAMCspNet, variant, pretrained, default_cfg=default_cfgs[variant],
        feature_cfg=dict(flatten_sequential=True), model_cfg=model_cfgs[cfg_variant], **kwargs)


@register_model
def CBAMcspresnet50(pretrained=False, **kwargs):
    return _create_cbamcspnet('CBAMcspresnet50', pretrained=pretrained, **kwargs)
