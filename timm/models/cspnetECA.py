...
__all__ = ['ECACspNet']  # model_registry will add each entrypoint fn to this
...
class ECACspNet(nn.Module):
    ...
    def __init__(self, cfg, in_chans=3, num_classes=1000, output_stride=32, global_pool='avg', drop_rate=0.,
                 ...
                 zero_init_last_bn=True, stage_fn=ECACrossStage, block_fn=ECABottleneck):
        super().__init__()
...
def _create_ecacspnet(variant, pretrained=False, **kwargs):
    cfg_variant = variant.split('_')[0]
    return build_model_with_cfg(
        ECACspNet, variant, pretrained, default_cfg=default_cfgs[variant],
        feature_cfg=dict(flatten_sequential=True), model_cfg=model_cfgs[cfg_variant], **kwargs)


@register_model
def ECAcspresnet50(pretrained=False, **kwargs):
    return _create_ecacspnet('ECAcspresnet50', pretrained=pretrained, **kwargs)
